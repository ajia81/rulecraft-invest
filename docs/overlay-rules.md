...
본 문서의 9단계는 CLAUDE.md에 명시된 8단계 파이프라인에서 두 가지 변경을 반영합니다. 첫째, `parse_yaml_blocks` 단계는 `load_skills` 단계의 일부로 통합되었습니다. Skills 파일을 로드하면서 동시에 마크다운에서 YAML 블록을 분리하는 것이 자연스럽고, 두 단계를 분리할 운영상의 이점이 없기 때문입니다. 둘째, `select_asset_overlay`와 `validate_merged_rules` 두 단계가 신규 추가되었습니다. `select_asset_overlay`는 본 문서 섹션 6의 자산군 분리 정책(다른 자산군의 오버레이는 평가하지 않는다)을 운영 단계로 표현한 것이며, `validate_merged_rules`는 섹션 5의 extend 검증 규칙(signal/severity/visualization 일치) 및 섹션 8의 inherit 룰 최소 구조 검증을 운영 단계로 표현한 것입니다. 두 단계 모두 룰 병합 후 실제 계산 단계로 진입하기 전에 룰의 일관성을 보장하기 위한 검증 게이트입니다.
